...
createReceiptsTable = function (baseUrl){
    ...
    return jQuery("#receipt-datatable").DataTable(config);
};
	      ...
	      buttons: {
		     "Submit": function() {
		    	 if( $('#ReceiptForm').valid()){
			    	 var filename = $("#file-select").get(0).files[0].filename; 
...
$("document").ready(function(){
    ...
    var table = createReceiptsTable(baseUrl);
    var dialog = createUploadReceiptDialog(baseUrl, table);
