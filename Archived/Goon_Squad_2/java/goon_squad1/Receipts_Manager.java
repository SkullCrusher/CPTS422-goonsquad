...
public class Receipts_Manager extends HttpServlet{	
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		String existing_id = req.getParameterMap().containsKey("id") ? req.getParameter("id") : "";
		...
		try{
			if (existing_id.length()>0) {
				query = "SELECT * FROM receipts WHERE `id` = "+existing_id;
			} else {
				query = "SELECT * FROM receipts WHERE `created_user_id` = "+req.getParameter("user_id");
			}
